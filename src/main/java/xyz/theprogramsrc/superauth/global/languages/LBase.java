...
public enum LBase implements TranslationPack {
    ...
    GUI_HOW_TO_USE("&aTo authenticate yourself an GUI will be opened, if you cannot see the GUI use the command /$1 and enter your pin."),
    COMMAND_HOW_TO_USE("&aTo register use the command /$1 <Password>. To login use the command /$2 <Password>"),
    REGISTER_COMMAND_USAGE("&cUse &e/register <Password> &cto register."),
    LOGIN_COMMAND_USAGE("&cUse &e/login <Password> &cto login."),

    ERROR_WHILE_HASHING("&cThe hashing util is not working right now, try again later."),
    WRONG_CAPTCHA("&cWrong captcha! &aEnter this captcha: &b$1"),
