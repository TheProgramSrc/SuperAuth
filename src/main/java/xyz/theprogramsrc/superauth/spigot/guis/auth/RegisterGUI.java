...
public class RegisterGUI extends GUI {
    ...
    protected String getTitle() {
        if(this.input == null){
            return LBase.AUTH_REGISTER_GUI_TITLE + " &9» " + LBase.NO_INPUT.options().upper().get();
        }else{
            return LBase.AUTH_REGISTER_GUI_TITLE + " &9» " + (this.input.isEmpty() ? LBase.NO_INPUT.options().upper().get() : this.input);
        }
    }
    ...
    private void finish(ClickAction a){
        if(this.authSettings.isCaptchaEnabled()){
            ...
            if(random <= this.authSettings.getCaptchaChance()){
                this.close();
                new CaptchaGUI(a.getPlayer(), true);
            }else {
                new ActionManager(a.getPlayer()).after(false);
            }
        }else{
            new ActionManager(a.getPlayer()).after(false);
        }
