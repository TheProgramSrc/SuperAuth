...
public class RegisterGUI extends Gui {
    ...
    public RegisterGUI(Player player, User user){
        super(player);
        this.input = "";
        ...
        this.user = user;
        this.canCloseGui = !this.player.isOnline() && this.user.isAuthorized();
        this.open();
    }
    ...
    public GuiTitle getTitle() {
        if(this.input == null){
            return GuiTitle.of(LBase.AUTH_REGISTER_GUI_TITLE + " &9» " + LBase.NO_INPUT.options().upper().get());
        }else{
            return GuiTitle.of(LBase.AUTH_REGISTER_GUI_TITLE + " &9» " + (this.input.isEmpty() ? LBase.NO_INPUT.options().upper().get() : this.input));
        }
    }
    ...
    private GuiEntry getDoneButton(){
        SimpleItem item = new SimpleItem(XMaterial.EMERALD)
                .setDisplayName("&a"+ LBase.AUTH_GUI_SAVE_PASSWORD_NAME + " »")
                .setLore(
                        "&7",
                        "&7" + LBase.AUTH_GUI_SAVE_PASSWORD_DESCRIPTION
                );
        return new GuiEntry(item, a->{
            try{
    ...
    private void finish(GuiAction a){
        if(this.authSettings.isCaptchaEnabled()){
            ...
            if(random <= this.authSettings.getCaptchaChance()){
                this.close();
                new CaptchaGUI(a.player, true);
            }else {
                new ActionManager(a.player).after(false);
            }
        }else{
            new ActionManager(a.player).after(false);
        }
